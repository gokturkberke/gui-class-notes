...
class ToDoList(ttk.Window):
    ...
    def create_widgets(self):
        ...
        self.tv.bind("<<TreeviewSelect>>", self.on_item_select) # bir oge secildiginde tetiklenecek fonksiyon
        self.entry_item.bind("<Return>", lambda e : self.add_item())

        # Populate the Treeview(Treeview widget'ını doldurmak için listeyi yeniliyoruz)
        self.refresh_list()
    ...
    def enable_actions(self):# Silme ve durum değiştirme butonlarını etkinleştiriyoruz
    ...
    def on_item_select(self, event):
        ...
        if selected_item:
            ...
            if item_status == "Done":
                self.status_var.set(1) # Mark as completed
            else:
    ...
    def toggle_status(self):
        ...
        if selected_item:
            ...
            if self.status_var.get() == 1:
                ...
                new_status = "Pending"
            self.db.update_status(item_id, new_status)# Durum veritabanında güncellenir
            self.refresh_list() # Listeyi yeniler
